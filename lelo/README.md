<img src="public/brand/logo.svg" alt="Lelo — know today, today" height="64">

# Lelo

**Know today, today.** One short message a day, for K1 — on any phone in Zambia.

*Lelo* means **today** in Bemba and Nyanja. That is the whole promise: not
yesterday's news, but the one thing that changes what you do today — the rain
coming to your field, the price at your market, the copper price, the fuel
price, the deadline that closes on Friday. See [`docs/BRAND.md`](docs/BRAND.md).
...
| Live USSD simulator on the landing page (demo mode only) | ✅ |
| Brand: name, mark, palette, and dependency-free icon generation | ✅ |
...
├── public/            landing page + live USSD simulator
│   ├── brand/         logo lockup and mark (see docs/BRAND.md)
│   └── assets/        generated PNG icons (npm run icons)
├── test/              45 tests
├── scripts/genicons.js  dependency-free PNG icon renderer
└── docs/              ARCHITECTURE · BRAND · COMPLIANCE · UNIT_ECONOMICS · ROADMAP
```
...
| `LELO_DB` | `./data/…` | **Leave unset on Vercel** — use Supabase there |

---

## Deploying

**Vercel.** `vercel.json` routes every request to `api/index.js` (the Express
app serves the static site too) and schedules the daily run at `0 4 * * *` UTC
— 06:00 Central Africa Time, which is when the bulletin should land. Set every
secret as an environment variable; the cron route authenticates with
`CRON_SECRET`.
...
Supabase project. Nothing written in demo mode outlives the instance — that is
the point, not a limitation. The landing page only shows its USSD simulator in
demo mode; against a real database that widget would be an unauthenticated way
for anyone to create subscriber rows.
