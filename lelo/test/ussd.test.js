...
test('USSD screens are normalised to GSM-7', () => {
  const content = require('../server/content');
  const out = ussd.respond(true, 'Lelo \u2014 K1/day\u2026');
  assert.equal(out.text, 'CON Lelo - K1/day...');
  assert.equal(content.isGsm7(out.text), true);
});
