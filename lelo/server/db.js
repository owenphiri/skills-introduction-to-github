...
const inMemory = config.dbFile === ':memory:';
if (!inMemory) fs.mkdirSync(path.dirname(config.dbFile), { recursive: true });
const db = new DatabaseSync(config.dbFile);
// WAL is a file-mode concern; an in-memory database has no journal to move.
...
db.exec(`
  ...
  -- Lelo content and nothing else, and cannot be sent to another
