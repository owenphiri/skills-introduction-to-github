...
function providerFor(channel) {
  if (channel === 'whatsapp') {
    // With no credentials there is nothing to call. Fall back to the mock so
    // the WhatsApp path stays demonstrable and testable rather than failing
    // every send with "credentials not set" — the same property the SMS and
    // payment adapters already have.
    const { phoneNumberId, accessToken } = config.whatsapp;
    return phoneNumberId && accessToken ? providers.whatsapp : providers.mock;
  }
...
async function send({ subscriber, channel, body, template = null, date = db.today(), allowDuplicate = false }) {
  ...
  // Content is normalised on ingest, but system copy — menus, replies, the
  // low-balance nudge — is written by hand and never passed through the
  // editor. Normalising here means no hand-written string can quietly force
  // UCS-2 and double its own cost. WhatsApp is UTF-8, so it keeps its
  // typography.
  if (channel === 'sms') body = content.toGsm7Safe(body);
